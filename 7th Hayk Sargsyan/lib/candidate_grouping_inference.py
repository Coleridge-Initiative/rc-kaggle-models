...
def is_word_overlap(str1, str2, threshold=0.5, ignore_connecting_words=True, n_call=1):
...
def exclude_candidate_with_special_symbols(candidate):
    ...
    for exclusion_symbol in exclusion_symbols:
        if exclusion_symbol in candidate:
            return True
...
def group_for_same_abbreviation(abb, candidates):
    ...
    for title in groups.copy():
        if exclude_group(groups[title], title):
            del groups[title]
    return groups


def group_test_candidates(candidates, sentences_candidates, title_labels):
    ...
    for abb in candidates:
        group = group_for_same_abbreviation(abb, candidates[abb])
        if len(group) >= 1:
    ...
    for k in dataset.copy():
        ...
        if definition not in dataset[k]:
            for cand in dataset[k]:
                if "(" in cand:
                    if f"{cand}" in dataset:
                        dataset[f"{cand}"].extend(dataset[k])
                        keys_to_remove.add(k)
                    else:
                        dataset[f"{cand}"] = [item for item in dataset[k]]
                        keys_to_remove.add(k)
                        break
        ...
        for k, v in label_dataset.items():
            for i in range(1, len(v)):
                ...
                if v[0] not in dataset:
                    continue
                dataset[v[0]].extend(dataset[v[i]])
                dataset[v[0]] = list(set(dataset[v[0]]))
                del dataset[v[i]]
    ...
    while True:
        for cand in label_dataset:
            if len(label_dataset[cand]) > 1:
                ...
                for title, abb, definition in zip(titles[1:], abbs[1:], definitions[1:]):
                    if abb == abbs[0]:
                        to_be_merged[titles[0]].add(title)
        ...
        for k, titles in to_be_merged.items():
            for title in titles:
                try:
                    dataset[k].extend(dataset[title])
                    del dataset[title]
                except KeyError:
                    continue
        label_dataset = expand_labels(dataset)
    ...
    for sentence_candidate in sentences_candidates:
        ...
        for reg_delimiter in reg_delimiters:
            ...
            for match in matches:
                ...
                if clean_label(candidate_1_2) in label_dataset_mapping:
                    title = label_dataset_mapping[clean_label(
                        candidate_1_2)]["group_name"]
                dataset[title].extend(construct_new_candidates(
        ...
        for title2 in dataset.copy():
            ...
            if abb1.lower() == abb2.lower():
                if to_merge(dataset[title1], dataset[title2], abb1, abb2):
                    dataset[title1].extend(dataset[title2])
                    del dataset[title2]
    ...
    for k, v in dataset.items():
        ...
        for cand in v:
            if cand.count("(") == 1:
                ...
                abbs_counter[abb] += 1
        try:
            most_common = abbs_counter.most_common()[0][0]
        except IndexError:
            continue
    ...
    for k, v in dataset.items():
        ...
        for abb in abbs_counter:
            if abb in most_common_abbs and abb != most_common:
                continue
            abbs.add(abb)
    ...
    for title in title_mapping:
        ...
        dataset[titles[0]].extend(title_labels[title])
